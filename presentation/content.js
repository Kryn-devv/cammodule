...
window.NOVA_CONTENT = (() => {
  ...
  const config = {
    ...
    project: 'IRIS',              // the open-source project it is built on
  ...
  const PROV_DOWN = 2;   // the provider that is rate-limited in the story
  const PROV_NEXT = 3;   // the one that takes over
  ...
  const scenes = [
    {
      ...
      notes: [
        ...
        `${config.name} is our name for the brain; it is built on the open-source project ${config.project}.`,
    ...
    },
    {
      ...
      lede: `${config.project} is a full desktop assistant that lives on your machine, not in the cloud. Say the wake word, then just speak.`,
      lines: [
        ...
        'Zero API keys needed for everyday commands',
        'Files and audio stay on your machine by default',
      ],
      ...
      notes: [
        ...
        'Free-first and local-first: no API key is needed for everyday commands; audio and files stay on the machine.',
      ],
    },
    {
      ...
      lede: 'The first layer that can handle a sentence, does. Cheap first, expensive last — the brain only wakes up when nothing else can answer.',
      lines: [
      ...
      notes: [
        ...
        'Layers 1–4 are deterministic and offline — wake word, memory commands, small talk, and 80 NLU intent rules that dispatch tools in milliseconds.',
    ...
    },
    {
      ...
      heading: 'Is it an agent? <span class="hl">Yes.</span>',
      lede: 'A chatbot: text in, text out, one step, done. An agent decides, acts through tools, checks the result and loops until the job is done.',
      lines: [
        ...
        '<span class="soft">“open YouTube” hits one NLU rule and runs in milliseconds. “Is this apple ripe?” hits none of the 80, so it is handed to this loop.</span>',
      ],
      chips: ['<b>86 tools</b> exposed as functions', 'multi-step tasks', 'layer 5 of the kernel'],
      labels: [
      ...
      bead: { path: circle(LOOP_R, 16, 0.25, 90, -1).concat([ringPt(LOOP_R, 90, 0.25)]),
        labelAt: [[0.25, 'plan: “I need to look at it”'], [0.5, 'act: call the camera tool'], [0.75, 'observe: one JPEG came back'], [1.1, 'done? → yes, answer']] },
      notes: [
        ...
        'Layer 5 of the kernel is exactly this loop — the LLM plans, tools act, results feed back — using function calling over all 86 tools.',
        'Layers 1–4 exist so this expensive loop only runs when it is needed.',
      ],
    },
    {
    ...
    {
      id: 'brain', label: 'Brains', len: 150, align: 'left', shape: 'providers', spin: 0.05,
      q: 'How does it work?', eyebrow: 'The brain',
      cam: { pos: [0, 0.5, 11.4], look: [0, 0.15, 0], drift: [0, -0.5, -0.4] }, shift: 2.0,
      heading: 'Eight brains, <span class="cool">one router</span>',
      lede: `${config.project} does not depend on a single model provider.`,
      lines: [
        'Tries your providers in your preferred order',
        'If one rate-limits, it fails over automatically',
        'A circuit breaker cools that provider down for a while',
        'No provider at all? Local commands still work offline',
      ],
      chips: PROVIDERS.map(p => `${p}`).concat(['<span class="soft">failover shown here is illustrative</span>']),
      labels: PROVIDERS.map((p, i) => { const k = (i === 6) ? 0.66 : 1.22; return { text: p, pos: [PROV_POS[i][0] * k, PROV_POS[i][1] * k, 0], c: i === PROV_DOWN ? P.danger : (i === PROV_NEXT ? P.ok : P.peri) }; })
        .concat([{ text: 'router', pos: [0, -0.75, 0.4], c: P.peri, cls: 'plain' }]),
      links: PROV_POS.map((p, i) => ({ a: [0, 0, 0], b: p, c: i === PROV_DOWN ? P.danger : P.faint })),
      bead: { path: [[0, 0, 0.3], PROV_POS[PROV_DOWN].map((v, k) => k === 2 ? 0.3 : v * 0.9), [0.2, -0.2, 0.3], PROV_POS[PROV_NEXT].map((v, k) => k === 2 ? 0.3 : v * 0.9)],
        labelAt: [[0.35, 'router → 1st choice'], [0.62, 'rate-limited → cool-down, try the next'], [1.1, '2nd choice answers ✓']] },
      notes: [
        'The LLM is not one vendor: the router tries the configured providers in the user’s preferred order.',
        'If one rate-limits, a circuit breaker cools it down and the next provider answers — automatic failover.',
        'With no provider at all, layers 1–4 still work offline. Basic commands need zero API keys.',
      ],
    },
    {
    ...
    },
    {
      ...
      lede: 'Three ESP32 boards on WiFi turn the assistant into a robot. Commands travel as UDP datagrams in milliseconds.',
      lines: [
        '<b>Motor board</b> (BTS7960) — drives the wheels',
      ...
      labels: [
        ...
        { text: 'BTS7960 motors', pos: [1.75, -1.25, 0.3], c: P.soft },
        { text: 'WiFi · UDP', pos: [0.35, 2.3, 0], c: P.amber },
      ],
      notes: [
        'The assistant has a body: three ESP32 boards on WiFi. Commands are UDP datagrams, so latency is milliseconds.',
        'Motor board (BTS7960) moves the robot; the ESP32-S3 sensor board has OLED eyes, ultrasonic, DHT22 temperature/humidity, PIR motion, gas, flame and light sensors.',
    ...
    },
    {
      ...
      lines: [
        '<code>GET /capture</code> — one fresh JPEG. This is what the brain calls',
        '<code>GET :81/stream</code> — live MJPEG on its own port, so a viewer can never block the brain',
      ...
      labels: [
        { text: 'OV2640 lens', pos: [0.95, 0.35, 0.4], c: P.amber },
      ...
      notes: [
        ...
        'GET /capture returns one fresh JPEG — that is the call the brain makes. The MJPEG stream lives on port 81 so a viewer can never block the brain.',
    ...
    },
    {
      ...
      steps: [
        'Wake word heard: “hey iris” <small>layer 1 · the mic opens</small>',
        'Not a memory command, not small talk, no NLU rule knows “ripe” <small>layers 2–4 pass it on</small>',
        'Agent loop: <b>plan</b> → call the camera tool <small>layer 5 · the LLM picks a tool</small>',
        '<code>GET http://robot-eye.local/capture</code> → one JPEG <small>the eye answers in one HTTP request</small>',
        'Frame → base64 → vision model: “list the objects; judge whether any fruit looks ripe” <small>act</small>',
    ...
    },
    {
      ...
      lines: [
        ...
        '<b>HIGH_RISK_ACTION</b> — power commands and the like, guarded',
      ...
      notes: [
        ...
        'Remote access (phone pairing by QR, Telegram) requires a bearer token and is rate-limited. Local-first by default.',
      ],
      labels: [
    ...
    },
    {
      ...
      cam: { pos: [0, -2.0, 9.4], look: [0, -2.0, 0], drift: [0, 0, -0.6] },
      html: `
        <p class="eyebrow"><span class="q">So…</span> is it an agent?</p>
        <h2><span class="hl">Yes.</span> It perceives, plans, acts through tools, observes — and loops until done.</h2>
        <div class="chips">
          <span><b>Perceives</b> · voice, camera, sensors</span>
        ...
        <p class="credits"><b>${config.team.join(' · ')}</b><br>${config.className} · built on ${config.project} (open source) · eye firmware: cammodule</p>
        <button class="again" id="btnAgain">Play it again</button>`,
      notes: [
        'The verdict in one sentence: yes, it is an agent — it perceives, plans, acts through tools, observes, and loops until done.',
